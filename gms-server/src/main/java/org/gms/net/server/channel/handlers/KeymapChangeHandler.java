                ...
                for (int i = 0; i < numChanges; i++) {
                    ...
                    if (type == 1) {
                        ...
                        if (skill != null) {
                            ...
                            if (isBanndedSkill || (!c.getPlayer().isGM() && GameConstants.isGMSkills(skill.getId())) || (!GameConstants.isInJobTree(skill.getId(), c.getPlayer().getJob().getId()) && !c.getPlayer().isGM() && c.getPlayer().getSkillLevel(skill) < 1)) { //for those skills are are "technically" in the beginner tab, like bamboo rain in Dojo or skills you find in PYPQ
