    ...
    if (mode == -1) {
        ...
        if (eim.getProperty(stage.toString() + "stageclear") != null) {
            ...
            if (eim.isEventLeader(cm.getPlayer())) {
                ...
                if (state == -1) {           // preamble
                    cm.sendOk("嗨。欢迎来到 #b玩具塔副本阶段" + stage + "#k。在这个阶段，让你的队伍中的" + Math.min(eim.getPlayerCount(), 5) + "名成员站在那些箱子上，以形成正确的组合来解锁下一个阶段。只有一个玩家应该留在所需的箱子上以确定组合。");
                    ...
                    if (playersOnCombo == Math.min(eim.getPlayerCount(), 5) || cm.getPlayer().gmLevel() > 1) {
                        ...
                        cm.sendNext("看起来你还没有找到" + Math.min(eim.getPlayerCount(), 5) + "个箱子。请考虑不同的箱子组合。只允许站在箱子上的数量为" + Math.min(eim.getPlayerCount(), 5) + "个，如果你移动箱子可能不算作答案，请记住这一点。继续加油！");
                        cm.dispose();
                    }
                }
            } else {
