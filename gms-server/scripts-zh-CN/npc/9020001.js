    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (eim.getProperty(stage.toString() + "stageclear") != null) {
                ...
                if (cm.isEventLeader()) {
                    var numpasses = Math.max(0, eim.getPlayerCount() - 1);     // minus leader

                    if (cm.hasItem(4001008, numpasses)) {
                        if (numpasses == 0) {
                            cm.sendNext("你已通过这一关！我会为你开启传送门。有时间限制，请赶快进入！");
                        } else {
                ...
                if (!eim.isEventLeader(cm.getPlayer())) {
                    ...
                    cm.sendNext("嗨。欢迎来到第二阶段。在我旁边，你会看到一些绳子，在这些绳子中，有#b" + required + "个与传送你到下一阶段的传送门相连#k。你只需要让#b" + required + "名队伍成员找到正确的绳子然后挂在上面#k\r\n但是，如果你挂得太低，这不算作答案；请确保靠近绳子中间位置才算作正确答案。此外，你的队伍只允许有" + required + "名成员挂在绳子上，队伍的队长必须#b双击我来检查答案是否正确#k。现在，寻找正确的绳子挂上去吧！");
                    var combo = [];
                    ...
                    eim.setProperty(stgProperty + "_required", required);
                } else {
                    var required = eim.getIntProperty(stgProperty + "_required");
                    ...
                    if (accept) {
                        ...
                        eim.showWrongEffect();
                        cm.sendNext("看起来你还没有找到正确的绳子。请考虑不同的组合。只允许" + required + "名成员挂在绳子上，如果你挂太低，它可能不算作答案，所以请记住这一点。继续努力！");
                    }
                }
                ...
                if (!eim.isEventLeader(cm.getPlayer())) {
                    ...
                    var required = Math.min(eim.getPlayerCount(), 3);
                    cm.sendNext("嗨。欢迎来到第三阶段。在我旁边，你会看到一些平台。在这些平台中，#b" + required + "个与传送你到下一阶段的传送门相连#k。你只需要让#b" + required + "个队员找到正确的平台站上去#k\r\n但是，如果你站得太靠边，是不行的；请确保靠近平台的中间位置才算作正确答案。此外，你的队伍只允许有" + required + "名成员站在平台上。一旦他们在上面，队伍的队长必须#b双击我来检查答案是否正确#k。现在，寻找正确的平台吧！");
                    var combo = [];
                    for (var i = 0; i < stgAreas.length; i++) combo.push(0);
                    var pos = [];
                    for (var i = 0; i < stgAreas.length; i++) pos.push(i);
                    ...
                    if (accept) {
                        ...
                        eim.showWrongEffect();
                        cm.sendNext("看起来你还没有找到正确的平台。请考虑不同的组合。只允许在" + required + "名成员站在平台上，如果你站得太靠边它可能不算作答案，所以请记住这一点。继续努力！");
                    }
                }
                ...
                if (!eim.isEventLeader(cm.getPlayer())) {
                    ...
                    var required = Math.min(eim.getPlayerCount(), 3);
                    cm.sendNext("嗨。欢迎来到第四阶段。在我旁边，你会看到一些木桶。在这些木桶中，#b" + required + "个与传送你到下一阶段的传送门相连#k。你只需要让#b" + required + "个队员找到正确的木桶站上去#k\r\n但是，如果你站得太靠边，这不算作答案；请站在木桶的中间才算作正确答案。此外，你的队伍只允许有" + required + "名成员站在木桶上。队伍的队长必须#b双击我来检查答案是否正确#k。现在，寻找正确的木桶站上去吧！");
                    var combo = [];
                    for (var i = 0; i < stgAreas.length; i++) combo.push(0);
                    var pos = [];
                    for (var i = 0; i < stgAreas.length; i++) pos.push(i);
                    ...
                    if (accept) {
                        ...
                        eim.showWrongEffect();
                        cm.sendNext("看起来你还没有找到第" + required + "个木桶。请考虑不同的木桶组合。只允许" + required + "名成员站在木桶上，如果你站得太靠边，它可能不算作答案，所以请记住这一点。继续努力！");
                    }
                }
