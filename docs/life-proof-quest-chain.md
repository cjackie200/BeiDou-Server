...
- 替代完成：每个阶段中进行中的主线或附加试炼可以在对应提交 NPC 处消耗 10 个绝对音感
  （`4310000`）直接完成当前任务。原目标已经完成时仍按正常流程免费提交；附加试炼选择任务和
  阶段奖励任务不能使用绝对音感跳过。
- 路线锁定：首次领取任意阶段奖励后，锁定生命之证路线，禁止 AP 操作 HP/MP。
- 放弃任务：主线、选择任务、奖励任务和已完成的附加试炼禁止放弃。进行中的固定附加试炼
  `base+12..14` 可以放弃；放弃时清空本轮已选目标及其进度，并把对应选择任务恢复为可领取，
  供玩家重新选择目标。放弃不会删除或回退 `hp_challenge_state`，因此玩家首次领取生命之证后
  仍永久视为已进入生命之证路线，帮助菜单继续禁止洗 HP/MP。
...
提交当前任务时，服务端必须先显示当前任务的完成确认文案。确认完成后，如果下一可领取任务的
`startNpcId` 等于当前 NPC，且 `Quest.canStart` 成立，服务端可以直接打开下一任务领取或选择
对话。客户端任务状态通过本次完成包里的 `nextQuest` 刷新，不要求玩家关闭对话、重新双击 NPC
才能看到下一步。
...
第 2、3 个 selector 的可选项只依赖前一个可见固定槽位完成；隐藏桥不参与菜单可用性判断，避免
客户端已经显示 selector、服务端却返回“暂无可选择的试炼”。
...
显示原生任务完成图标或书本。玩家点击目标导师的任务入口后，服务端先显示目标导师的完成确认
文案；玩家确认后写入 `QuestStatus` 进度并完成该任务。如果下一步仍由当前目标导师领取，则在
完成确认之后继续打开下一步领取对话。不再使用 `4033011` 作为完成证明绕路道具。
...
T1 拜访教官任务中，目标导师承载生命之证完成入口。普通双击目标导师仍显示该 NPC 原本对话，
不会记录拜访进度；只有点击任务完成图标才会直接记录并推进下一步。

进行中对话采用原版多步骤任务风格，不再使用说明书式字段。对话必须包含：

- 标题：当前任务名，例如 `生命之证 I：拜访赫丽娜`。
- 阶段叙事：使用对应阶段故事文本，让对话仍然属于生命之证流程。
- 自然目标句：说明当前要拜访、击杀、收集、缴纳、选择或领奖的事项。
- 进度行：按任务类型显示已完成数量和需求数量。
- 自然行动句：告诉玩家继续完成、去哪个 NPC 提交，或条件已满足可以提交。
...
去见#p1012100#，让对方确认这一步生命之证。
与#p1012100#确认 #b0#k/#r1#k

前往#p1012100#，点击任务完成图标，由对方确认这一步生命之证。
```
...
把需要的物品收齐后，回#p1022000#提交。
#i4033001# #t4033001# #b8#k/#r20#k
...
生命之证 `KILL` 目标必须使用正常玩家能抵达、且单张地图有多只直接刷怪点的怪物；
`HpChallengeServiceTest.lifeProofKillTargetsUseFarmableMapSpawns` 会校验所有公共、职业和附加击杀目标
在 `Map.wz` 中至少存在 3 个直接刷怪点。
...
Q 任务详情 `QuestInfo.1` 使用原版多步骤任务详情风格：先写阶段背景，再用自然叙述说明当前步骤要做的
拜访、击杀、收集、附加试炼选择或领奖事项，末尾放置原生 `mob` 进度宏或 InteractionHook 动态进度
占位符。不得回退到只有“任务进度 + marker”的空描述，也不得使用
`任务目标：/目标：/当前目标：/当前进度：/完成方式：/下一步：` 等说明书式字段。
每个可见生命之证任务同时写入 `summary` 和 `demandSummary`，用于 Q 列表摘要和需求区展示。
任务标题已经写明的小步骤不得在正文或摘要里机械复读；例如标题是 `生命之证 I：拜访赫丽娜` 时，
正文写 `去见#p1012100#，请对方留下确认。`，不要再写 `拜访赫丽娜`。`summary` 使用类型化短句，
不能等于完整任务名；`demandSummary` 写 NPC、怪物、物品或通用需求。
完成文本不得以 `已完成：` 开头。奖励任务必须写清楚回到本职业一转教官领取本阶段生命之证和 HP/MP
保底记录，不能只写泛泛的“回去领取奖励”。
...
4005000 力量水晶: T2 公共主线 30 个；T7 公共主线 10 个
4033002 龙巢生命之证: T2 附加试炼 5, 40 个, 8140700/8140701/8140702/8140703
4005001 智慧水晶: T3 公共主线 30 个；T7 公共主线 10 个
4033004 玩具塔生命之证: T3 附加试炼 5, 60 个, 7140000/8141100/8140200/8140300
4005004 黑暗水晶: T4 公共主线 30 个；T7 公共主线 10 个
4005002 敏捷水晶: T5 公共主线 30 个；T7 公共主线 10 个
4033007 暗流生命之证: T5 附加试炼 6, 50 个, 8140555/8140600/8150100/8150101
4005003 幸运水晶: T6 公共主线 30 个；T7 公共主线 10 个
4033009 回忆生命之证: T6 附加试炼 7, 75 个, 8200005/8200006/8200007/8200008/8200009/8200010/8200011/8200012
T7 公共主线使用五种水晶各 10 个，Check.img 必须写 5 个 item gate，不能写成单个 4033011 x50
