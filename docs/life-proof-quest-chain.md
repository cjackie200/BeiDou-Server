...
其他入口、职业转职文本、商店和其他 NPC 功能必须保持原逻辑。T1 拜访目标 NPC 的进度由
服务端 `LifeProofQuest.onNpcTalk()` 在原始 NPC 点击流程中记录，记录后继续执行该 NPC 原始对话。
...
然后继续显示该 NPC 原本对话；目标 NPC 不承载生命之证进行中对话。
...
- `PQ_ANY`、`PQ_PIRATE`、`PQ_TOY_OR_PIRATE`、`SCROLL_100`、`JUMP_MANUAL` 和
  `NPC_TALK` 使用 `QuestStatus` 自定义进度，WZ 完成条件写入 `infoex` 门槛。未达到进度时，
  客户端任务列表必须保持进行中，不得显示可完成。
