...
- 任务列表归类：所有可见生命之证任务写 `QuestInfo.area=31`，`wz-zh-CN/Etc.wz/QuestCategory.img`
  和客户端 `Data/Etc/QuestCategory.img` 的 `31` 命名为 `传奇之路`。同阶段五职业共享
  `parent=生命之证 I..VII`，按本职业本阶段可见顺序写 `order`：T1 为 `1..21`，
  T2-T7 为 `1..18`。不同职业同阶段可以重复 `order`，由 WZ `job` 条件和服务端 branch 过滤。
...
- 所有可见生命之证任务在任务列表按 `生命之证 I..VII` 归类；T1 每职业 `order=1..21`，
  T2-T7 每职业每阶段 `order=1..18`。
